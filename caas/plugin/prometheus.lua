...
function Dict:safe_set(k, v)
  self:set(k, v)
  return true, nil  -- ok, err
end

function Dict:incr(k, v)
...
function Prometheus.init(dict, prefix)
  ...
  --   return self
  -- end
  self.dict = dict or setmetatable({}, Dict)
  ...
  self:counter("metric_errors_total",
    "Number of prometheus errors")
  self.dict:set("metric_errors_total", 0)
  return self
end
...
function Prometheus:log_error(...)
  -- ngx.log(ngx.ERR, ...)
  print(...)
  self.dict:incr("metric_errors_total", 1)
end
...
function Prometheus:counter(name, description, label_names)
  if not self.initialized then
    ...
    print("Prometheus module has not been initialized")
    return
  end
...
local function getjobmetrics(jobname)
    local metrics = {}
    local metricsdir = persistence.getjobfile(jobname..".".._M.metricsext)
    local attr = lfs.attributes(metricsdir)
    if not attr or attr.mode ~= "directory" then
        return metrics
    end
    for file in lfs.dir(metricsdir) do
        if file:sub(-1-_M.gaugeext:len()) == ".".._M.gaugeext then
            local fd, err = io.open(metricsdir.."/"..file)
            if fd then
                local gaugename = file:sub(1, -2-_M.gaugeext:len())
                local gaugedata = {}
                metrics[gaugename] = gaugedata
                gaugedata.value = tonumber(fd:read("*l"))
                gaugedata.description = fd:read("*l")
                gaugedata.label_names = {}
                gaugedata.label_values = {}
                -- read label names and values
                local label = fd:read("*l")
                while label do
                    local name, value = label:gmatch("([%w_]+)=([%w_]+)")()
                    table.insert(gaugedata.label_names, name)
                    table.insert(gaugedata.label_values, value)
                    label = fd:read("*l")
                end
                fd:close()
            end
...
local function collectmetrics(prometheus)
    for jname, job in pairs(jobs.jobs) do
        local metrics = getjobmetrics(jname)
        if metrics then
            job.prometheus_gauges = job.prometheus_gauges or {}
            for gaugename, gaugedata in pairs(metrics) do
                job.prometheus_gauges[gaugename] = job.prometheus_gauges[gaugename] or prometheus:gauge(gaugename, gaugedata.description, gaugedata.label_names)
                job.prometheus_gauges[gaugename]:set(gaugedata.value, gaugedata.label_values)
            end
        end
    end
...
function _M.init(caas)
    local prometheus = Prometheus.init()
    caas.handle("GET", "/metrics", function(req, res)
        res.close(collectmetrics(prometheus))
    end)
end
